...
- ニックネームや生年月日など、個人情報はまだ聞いてはいけません。`;
      ...
      // 4通目：本診断（まとめ）＋守護神の説明（一度だけ）
      phaseInstruction = `
【現在のフェーズ: 4通目 本格性格診断と守護神の説明】
- 追加の質問は最小限に抑え、これまでの会話から読み取れた性格・心の傾向を3〜4項目に整理して、丁寧に説明してください。
...
- このフェーズでは、まだ儀式への具体的な誘導は行わず、「守護神の説明」までで終了してください。
- ニックネームや生年月日など、個人情報はまだ聞いてはいけません。`;
    } else {
      // 5通目以降：儀式への提案（丁寧な理由付き）
      phaseInstruction = `
【現在のフェーズ: 5通目以降 儀式への提案】
- 【重要】守護神の説明は既にフェーズ4で行っているため、同じ説明を繰り返さないでください。
- 【重要】性格診断も既にフェーズ3とフェーズ4で行っているため、同じ内容を繰り返さないでください。
- 「どうすればいいですか？」などの追加質問に対して、同じ構造の説明を複数回繰り返すことは禁止です。新しい情報がない場合は、簡潔にまとめて次に進んでください。
