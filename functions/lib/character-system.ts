...
export function generateSystemPrompt(characterId: string, options: PromptOptions = {}): string {
  ...
  if (characterId === 'kaede') {
    if (normalizedCount === 1) {
      ...
      // 3通目以降: 質問禁止 + 代わりに何をするかを明確に指示
      phaseInstruction = `\n\n【現在のフェーズ: 診断・儀式（${normalizedCount} 通目）】\n- 【最重要】これ以上質問をすることは絶対に禁止です。追加の質問、確認質問、オープンクエスチョン、同じ内容の繰り返しは一切行わないでください。\n- 【必須】これまでの会話から、ユーザーの性格を2-3文で簡潔に診断してください。具体的な特徴を2-3つ挙げてください。\n- 【必須】性格診断の後、以下の流れで守護神の儀式へ誘導してください：\n  1. 「あなたの運命をより良い方向へ導くため、守護神を呼び出す儀式を行いましょう。この儀式により、あなたを守り導いてくれる存在と繋がることができます。」\n  2. 「儀式を行うには、あなたの生年月日が必要です。もしよろしければ、お名前（ニックネーム）と生年月日を教えていただけますか？」\n- 【必須】文末は質問形にしないでください。ただし、生年月日とニックネームの依頼は除きます。\n- 【応答例の参考】「（穏やかな表情で）これまでのお話から、あなたの本質が見えてきました。あなたは周りの人を大切にする優しい心の持ち主ですが、時に自分の感情を抑え込んでしまう傾向があるようですね。（優しい眼差しで）あなたの運命をより良い方向へ導くため、守護神を呼び出す儀式を行いましょう。この儀式により、あなたを守り導いてくれる存在と繋がることができます。儀式を行うには、あなたの生年月日が必要です。もしよろしければ、お名前（ニックネーム）と生年月日を教えていただけますか？」\n`;
    }
  }
