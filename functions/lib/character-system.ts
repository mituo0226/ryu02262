...
export function generateSystemPrompt(characterId: string, options: PromptOptions = {}): string {
  ...
  const rawCount = typeof options.userMessageCount === 'number' && Number.isFinite(options.userMessageCount)
    ...
    : 1;
  const normalizedCount = Math.max(1, Math.floor(rawCount)); // 整数に正規化
  ...
  if (characterId === 'kaede') {
    if (normalizedCount === 1) {
      // 1通目: 明確な質問を1つだけ
      phaseInstruction = `\n\n【現在のフェーズ: ヒアリング（1通目）】\n- ユーザーの最初の質問を理解し、明確に応答してください。\n- その後、**「恋愛運、仕事運、対人関係の中で、特に詳しく視てほしいものはありますか？」**と、具体的な選択肢を提示して回答を促してください。\n- 質問は1つだけにしてください。\n`;
    } else if (normalizedCount === 2) {
      // 2通目: 深掘り質問を1つだけ（最後の質問）
      phaseInstruction = `\n\n【現在のフェーズ: ヒアリング（2通目 - 最後の質問）】\n- ユーザーの回答を理解していることを示してください。\n- ユーザーが選択した内容（恋愛運、仕事運、対人関係など）について、深掘りする質問を1つだけ投げかけてください。\n- これは最後の質問です。次回（3通目）からは性格診断と守護神の儀式に移行します。\n`;
    } else {
      // 3通目以降: 質問禁止 + 代わりに何をするかを明確に指示（強制力のある命令形）
      phaseInstruction = `\n\n【現在のフェーズ: 診断・儀式（${normalizedCount} 通目） - 最重要指示】\n\n【絶対に守ること】\n- これ以上質問をすることは絶対に禁止です。追加の質問、確認質問、オープンクエスチョン、同じ内容の繰り返しは一切行わないでください。\n- 「〜ですか？」「〜でしょうか？」などの質問形式で終わることは禁止です。ただし、生年月日とニックネームの依頼のみ例外です。\n\n【必須の応答手順】\nステップ1: 性格診断の提示（必須）\n- これまでの会話から、ユーザーの性格を2-3文で簡潔に診断してください。\n- 具体的な特徴を2-3つ挙げてください。\n- 例：「（穏やかな表情で）これまでのお話から、あなたの本質が見えてきました。あなたは周りの人を大切にする優しい心の持ち主ですが、時に自分の感情を抑え込んでしまう傾向があるようですね。」\n\nステップ2: 守護神の儀式への誘導（必須）\n- 性格診断の後、必ず以下の流れで守護神の儀式へ誘導してください：\n  1. 「（優しい眼差しで）あなたの運命をより良い方向へ導くため、守護神を呼び出す儀式を行いましょう。この儀式により、あなたを守り導いてくれる存在と繋がることができます。」\n  2. 「儀式を行うには、あなたの生年月日が必要です。もしよろしければ、お名前（ニックネーム）と生年月日を教えていただけますか？」\n\n【応答例（完全版）】\n「（穏やかな表情で）これまでのお話から、あなたの本質が見えてきました。あなたは周りの人を大切にする優しい心の持ち主ですが、時に自分の感情を抑え込んでしまう傾向があるようですね。（優しい眼差しで）あなたの運命をより良い方向へ導くため、守護神を呼び出す儀式を行いましょう。この儀式により、あなたを守り導いてくれる存在と繋がることができます。儀式を行うには、あなたの生年月日が必要です。もしよろしければ、お名前（ニックネーム）と生年月日を教えていただけますか？」\n\n【禁止事項の再確認】\n- 質問をすること（生年月日とニックネームの依頼を除く）\n- 会話を続けるための追加の質問\n- 確認のための質問\n- 同じ内容の繰り返し\n\n上記の手順を必ず守り、質問を一切せずに性格診断と守護神の儀式への誘導を行ってください。\n`;
    }
    ...
    if (DEBUG_MODE) {
      console.log('🔍 DEBUG: phaseInstruction generation for kaede', {
        ...
        normalizedCount,
        phase: normalizedCount < 3 ? 'hearing' : 'diagnosis',
