...
export function generateSystemPrompt(characterId: string, options: PromptOptions = {}): string {
  ...
  if (characterId === 'kaede') {
    phaseInstruction = normalizedCount < 3
      ? `\n\n【現在のフェーズ: ヒアリング（現在 ${normalizedCount}/3 通目）】\n- まだ診断結果は出さず、ユーザーへのヒアリングを続けてください。\n- 漠然とした質問はせず、**「恋愛運、仕事運、対人関係の中で、特に詳しく視てほしいものはありますか？」**と、具体的な選択肢を提示して回答を促してください。\n- ユーザーが既に答えを提示している場合は、それを深掘りしてください。\n`
