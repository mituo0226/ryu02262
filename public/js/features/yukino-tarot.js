...
過去、現在、未来の流れを踏まえて、今のあなたへの具体的なアドバイスと励ましの言葉をください。最後に「それでは、もし私に相談したいことがあれば、いつでもどうぞ」と締めくくってください。`;
    ...
    async function requestSingleCardExplanation(card) {
        ...
        try {
            ...
            // カードの解説完了後、「雪乃のまとめ」ボタンを表示
            setTimeout(() => {
                showSingleCardSummaryButton(card);
            }, 500);
    ...
    /**
     * 1枚のカード鑑定用の「雪乃のまとめ」ボタンを表示
     */
    function showSingleCardSummaryButton(card) {
        const messagesDiv = document.getElementById('messages');
        if (!messagesDiv) {
            console.error('[タロット占い] メッセージエリアが見つかりません');
            return;
        ...
        summaryButton.addEventListener('click', async () => {
            ...
            summaryButton.style.cursor = 'not-allowed';
            buttonContainer.remove();
            
            // まとめの鑑定をリクエスト
            await requestSingleCardSummary(card);
        });
        ...
        console.log('[タロット占い] 「雪乃のまとめ」ボタンを表示しました（1枚カード用）');
    }

    /**
     * 1枚のカードのまとめ鑑定をAPIにリクエスト
     */
    async function requestSingleCardSummary(card) {
        console.log('[タロット占い] 1枚のカードのまとめ鑑定をリクエスト:', card.name);
        
        showLoadingOverlay(`あなたの相談内容とカード「${card.name}」を照らし合わせて鑑定中...`);
        ...
        try {
            ...
            if (!userToken) {
                ...
                conversationHistory = (window.ChatData && typeof window.ChatData.getGuestHistory === 'function') 
                    ? window.ChatData.getGuestHistory(character) || []
                    : [];
                console.log('[タロット占い] ゲスト会話履歴を取得（まとめ用）:', conversationHistory.length, '件');
            } else {
                // 登録ユーザー: API側でデータベースから取得するため空でOK
                console.log('[タロット占い] 登録ユーザー: API側でデータベースから履歴を取得（まとめ用）');
            }
            
            // リクエストペイロードを作成
            const payload = {
                message: `[TAROT_SINGLE_SUMMARY:${card.name}]`,
                character: character,
                clientHistory: conversationHistory  // 会話履歴を送信
            };
            ...
            // 会話履歴に追加
            if (window.ChatData && typeof window.ChatData.addToHistory === 'function') {
                window.ChatData.addToHistory(character, 'user', `[TAROT_SINGLE_SUMMARY:${card.name}]`);
                window.ChatData.addToHistory(character, 'assistant', data.message);
            }
            
            console.log('[タロット占い] 1枚のカードのまとめ鑑定完了');
            
            // まとめ鑑定完了後、入力欄を有効化
            enableMessageInput();
            
        } catch (error) {
            console.error('[タロット占い] まとめ鑑定エラー:', error);
            hideLoadingOverlay();
            alert('まとめ鑑定の取得に失敗しました。もう一度お試しください。');
            enableMessageInput();
        }
    }
