    ...
    async sendMessage(skipUserMessage = false, skipAnimation = false, messageOverride = null) {
        ...
        try {
            ...
            if (isGuest) {
                ...
                // タロットカード解説トリガーの場合の特別処理
                if (isTarotExplanationTrigger) {
                    // タロットカード解説トリガーは会話履歴に追加されないため、
                    // currentCountは実際のユーザーメッセージ数を表している
                    // API側では「初回メッセージ送信後、カード解説が何度も実行される」ため、
                    // currentCountをそのまま使用する（-1しない）
                    messageCountForAPI = currentCount;
                } else {
                    // 通常のメッセージの場合、会話履歴には既に今回送信するメッセージが含まれているため、
                ...
                console.log('🔍🔍🔍 [原因調査] APIに送信するメッセージカウント:', {
                    ...
                    期待されるフェーズ: messageCountForAPI + 1 === 1 ? 'フェーズ1（初回挨拶）' : 'フェーズ2以降（カード解説/まとめ）',
